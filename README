...
2x.py -- Take 2 to the power of x.

clean_ports.py -- Selfupdates macports, checks for inactive ports, and 
	uninstalls inactive ports. Note that this program assumes that macports is 
	installed. I don't know what would happen if you tried to run this without 
	having macports installed.

dump_columns.py -- Dumps two columns from a FITS file into an ASCII/txt/dat
	file.

fits_info.py -- Prints HDU information, extension 1 header keywords, and 
	extension 1 column names for a given FITS file. Using extension 1 here for 
	RXTE event-mode data.

get_keyword.py -- Gets the value of a keyword from a FITS header.

obs_epoch_rxte.py -- Determines the observational epoch of a specific RXTE 
	observation. Be careful with this one, it's barely tested.

total_obs_time.py -- Computes the total observation time for a list of FITS 
	observation files.

tools.py -- centralized helper methods to import and call within another 
	function:
